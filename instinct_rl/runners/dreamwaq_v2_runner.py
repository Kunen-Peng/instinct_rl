...
class DreamWaQRunnerV2(OnPolicyRunner):
    def __init__(self, env, train_cfg, log_dir=None, device="cpu"):
        ...
        # Ensure we use PPODreamWaQV2
        alg_class = PPODreamWaQV2
...
class DreamWaQRecurrentRunnerV2(DreamWaQRunnerV2):
    """
    ...
    def __init__(self, env, train_cfg, log_dir=None, device="cpu"):
        ...
        if self.cenet.rnn is not None:
            self.cenet_hidden_states = torch.zeros(
                self.cenet.rnn.num_layers, self.env.num_envs, self.cenet.rnn.hidden_size, device=self.device
            )
        else:
        ...
        # Use PPODreamWaQRecurrentV2
        alg_class = PPODreamWaQRecurrentV2
