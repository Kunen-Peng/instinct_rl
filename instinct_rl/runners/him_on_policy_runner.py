...
class HIMOnPolicyRunner:
    """Runner for Hierarchical Imitation Mode (HIM) training.
    
    This runner extends the standard OnPolicyRunner with support for:
    - Next critic observations from termination observations
    - More accurate value bootstrapping for terminated episodes
    - Compatible with HIMPPO algorithm
    """
            ...
            if term_obs_format is not None:
                for obs_group_name, config in self.cfg.get("normalizers", dict()).items():
                    if obs_group_name in term_obs_format:
                        # Termination obs will share the same normalizer as regular obs
                        pass
    ...
    def rollout_step(self, obs, critic_obs):
        ...
        This method is optimized to work with ObservationManager's history buffer.
        Key points:
        1. Policy observations (obs) already contain full history from CircularBuffer
           Format: [obs_t0, obs_t1, ..., obs_t(H-1)] (oldest_first)
        2. HIMActorCritic and HIMEstimator are designed to work with this format
        3. No additional history storage or ordering is needed
        
        Returns:
            obs: Policy observations (with history from CircularBuffer)
        ...
        # obs here is already the full history from ObservationManager
        ...
        # At this point, obs and critic_obs are flattened tensors from ObservationManager
        for obs_group_name, normalizer in self.normalizers.items():
            if obs_group_name == "policy":
                # Normalize policy observations (which include full history)
                obs = normalizer(obs)
                infos["observations"]["policy"] = obs
            elif obs_group_name == "critic":
                # Normalize critic observations if present
                critic_obs = normalizer(critic_obs)
                infos["observations"]["critic"] = critic_obs
            else:
                # Normalize any other observation groups
        ...
        # This creates a modified version of critic_obs with termination observations
        # for environments that exceeded max episode length
        ...
        if self.use_termination_obs and critic_obs is not None:
            ...
            if len(termination_env_ids) > 0 and len(termination_obs) > 0:
                # Create next_critic_obs_for_bootstrap by cloning current critic_obs
                next_critic_obs_for_bootstrap = critic_obs.clone().detach()
                
                # Get termination critic observations and replace them
                ...
                if term_critic_obs is not None:
                    ...
                    # Apply normalizer to termination observations if needed
                    if "critic" in self.normalizers:
                        term_critic_obs = self.normalizers["critic"](term_critic_obs)
                    # Replace observations for terminated environments with termination obs
                    next_critic_obs_for_bootstrap[termination_env_ids] = term_critic_obs.clone().detach()
        
        # Process the environment step
        # Note: obs now contains the full observation history from ObservationManager
        # HIMActorCritic.update_distribution() expects this format and will extract
        # the newest observation (last num_one_step_obs elements) for the current state
