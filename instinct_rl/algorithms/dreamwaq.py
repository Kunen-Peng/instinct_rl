...
class PPODreamWaQ(PPO):
    ...
    def act(self, obs, critic_obs):
        ...
        obs_augmented = torch.cat((obs, estimate), dim= -1)
    ...
    def process_env_step(self, rewards, dones, infos, rewards_noClip, single_obs, next_obs, next_critic_obs, next_critic_obs_for_bootstrap=None):
        self.transition.single_obs = single_obs.clone()
    ...
    def update(self, current_learning_iteration):
        ...
        for minibatch in generator:
            ...
            # Estimator Update
            if self.compute_Pboot():
                # Extract original obs (remove augmented estimate)
                lin_vel = get_subobs_by_components(minibatch.critic_obs, ["base_lin_vel"], 
                                                 self.actor_critic.critic_obs_segments)
                
                # Strip estimate from obs
                raw_obs = minibatch.obs[..., :self.cenet.encoder.model[0].in_features] 
                
                self.cenet.encode(raw_obs)
                est_mean = self.cenet.encoder_mean
                est_mean.requires_grad_(True)
                est_logvar = self.cenet.encoder_logvar
                
                kl_loss = torch.mean(-0.5 * torch.sum(1 + est_logvar[:,:-16] - est_mean[:,:-16] ** 2 - torch.exp(est_logvar[:,:-16]), 1), 0)
                
                loss_vt = torch.nn.functional.mse_loss(est_mean[:,:3], lin_vel)
                loss_ot = torch.nn.functional.mse_loss(self.cenet.decode(est_mean), minibatch.single_obs)
